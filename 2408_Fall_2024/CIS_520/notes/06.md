...
- e.g.) Probabilistic Inference
  ...
  3. Use gradient descent to maximize the conditional likelihood over the data set $`\mathcal{D}`$.
     - e.g.) $`\displaystyle \min_{h\in\mathcal{F}}\hat{R}(h) = \max_{h\in\mathcal{F}}\prod_i p(y_i\vert x_i,f)`$
    ...
    - Thus, $`\nabla_w \hat{R}(\mathbf{w}) = 0`$ will be the global minimum.
      - i.e.)   
        ...
        $`\begin{aligned}
            ...
            \Leftrightarrow & \mathbf{(X^\top X) w} = \mathbf{X^\top y} \\
            \Rightarrow & \mathbf{w} = \mathbf{(X^\top X)^{-1} X^\top y}
        \end{aligned}`$   
        - cf.) (1)   
          $`\begin{aligned}
...
### Concept) Overfitting
- Recall that our linear regression model derived the optimal model by $`\mathbf{w}_* = \mathbf{(X^\top X)^{-1} X^\top y}`$.
- Then $`\mathbf{Xw}_* = \mathbf{X(X^\top X)^{-1} X^\top y}`$.
...
- Relations between 1 and 2)
  ...
  - In general, converting between these two forms is not easy: it’s not always easy to determine what $`B`$ should be for a particular $`\lambda`$ and vice versa without just running the optimization.
