...
- Training Using AUROC)
  - The AUROC of $`f`$ on the test sample   
    $`\displaystyle \text{AUC}[f] = \frac{1}{n_{+}}\frac{1}{n_{-}}\sum_{i:y_i'=+1}\sum_{i:y_i' = -1} \mathbf{1}(f(x_i') \gt f(x_j'))`$
    - where
  ...
  - Optimization problem using the loss function $`\ell`$   
    $`\displaystyle \min_f \frac{1}{n_{+}}\frac{1}{n_{-}}\sum_{i:y_i=+1}\sum_{i:y_i = -1} \ell(+1, f(x_i) - f(x_j))`$
