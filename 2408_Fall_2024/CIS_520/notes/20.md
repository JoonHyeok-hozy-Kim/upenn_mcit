...
- Model)
  ...
  - Then we have
    - $`\mathbf{z} = f(\mathbf{x}) = \mathbf{Qx} = \begin{bmatrix} \mathbf{x}_i^\top\mathbf{q}_1 & \mathbf{x}_i^\top\mathbf{q}_2 & \cdots & \mathbf{x}_i^\top\mathbf{q}_d \end{bmatrix} \in \mathbb{R}^d`$
...
- Derivation)
  ...
  - Consider the dimensionality reduction where...
    - the encoder $`f`$ goes   
      - In vector form   
        $`\begin{aligned}
          ...
          &= \begin{bmatrix} \mathbf{q}_1^\top \\ \mathbf{q}_2^\top \\ \vdots \\ \mathbf{q}_d^\top \end{bmatrix} \mathbf{x}_i = \begin{bmatrix} \mathbf{q}_1^\top\mathbf{x}_i \\ \mathbf{q}_2^\top\mathbf{x}_i \\ \vdots \\ \mathbf{q}_d^\top\mathbf{x}_i \end{bmatrix} = \begin{bmatrix} \mathbf{x}_i^\top\mathbf{q}_1 & \mathbf{x}_i^\top\mathbf{q}_2 & \cdots & \mathbf{x}_i^\top\mathbf{q}_d \end{bmatrix} \in\mathbb{R}^d
        \end{aligned}`$
      - In matrix form   
        $`\begin{aligned}
          \mathbf{XQ}^\top 
          &= \begin{bmatrix} \mathbf{x}_1 \\ \mathbf{x}_2 \\ \vdots \\ \mathbf{x}_n \end{bmatrix} \begin{bmatrix} \mathbf{q}_1 & \mathbf{q}_2 & \cdots & \mathbf{q}_d \end{bmatrix} \\
    ...
    - the decoder $`g`$ goes   
      - In vector form   
        $`\hat{\mathbf{x}}_i = \mathbf{Q}^\top (\mathbf{Q}\mathbf{x}_i) = \begin{bmatrix} \mathbf{q}_1 & \mathbf{q}_2 & \cdots & \mathbf{q}_d \end{bmatrix} \begin{bmatrix} \mathbf{q}_1^\top\mathbf{x}_i \\ \mathbf{q}_2^\top\mathbf{x}_i \\ \vdots \\ \mathbf{q}_d^\top\mathbf{x}_i \end{bmatrix} = \begin{bmatrix} (\mathbf{q}_1^\top\mathbf{x}_i)\mathbf{q}_1 \\ (\mathbf{q}_2^\top\mathbf{x}_i)\mathbf{q}_2 \\ \vdots \\ (\mathbf{q}_d^\top\mathbf{x}_i)\mathbf{q}_d \end{bmatrix} \in\mathbb{R}^D `$
      - In matrix form   
        $`\begin{aligned}
          \mathbf{XQ^\top Q} 
          ...
          &= \begin{bmatrix} (\mathbf{x}_1^\top\mathbf{q}_1)\mathbf{q}_1^\top + (\mathbf{x}_1^\top\mathbf{q}_2)\mathbf{q}_2^\top + \cdots (\mathbf{x}_1^\top\mathbf{q}_d)\mathbf{q}_d^\top \\ (\mathbf{x}_2^\top\mathbf{q}_1)\mathbf{q}_1^\top + (\mathbf{x}_2^\top\mathbf{q}_2)\mathbf{q}_2^\top + \cdots (\mathbf{x}_2^\top\mathbf{q}_d)\mathbf{q}_d^\top \\ \vdots \\ (\mathbf{x}_n^\top\mathbf{q}_1)\mathbf{q}_1^\top + (\mathbf{x}_n^\top\mathbf{q}_2)\mathbf{q}_2^\top + \cdots (\mathbf{x}_n^\top\mathbf{q}_d)\mathbf{q}_d^\top \\  \end{bmatrix} \in\mathbb{R}^{n\times D}
        \end{aligned}`$
  - Also, consider the projection of $`\mathbf{x}_i`$ onto $`\mathbf{q}_j`$ is given by
    ...
    - This is identical to the $`j`$-th row of the decoder $`\hat{\mathbf{x}}_i`$ where   
      $`\hat{\mathbf{x}}_i = \begin{bmatrix} (\mathbf{q}_1^\top\mathbf{x}_i)\mathbf{q}_1 \\ \vdots \\ (\mathbf{q}_j^\top\mathbf{x}_i)\mathbf{q}_j \\ \vdots \\ (\mathbf{q}_d^\top\mathbf{x}_i)\mathbf{q}_d \end{bmatrix}`$
  ...
  - How to get the maximum variance of $`\mathbf{z}_i`$
    - The sample variance for $`\mathbf{z}_i`$ w.r.t. $`\mathbf{q}_j`$ is given by   
      $`\begin{aligned}
        \frac{1}{n-1}\sum_{i=1}^n\mathbf{z}_i^2 &= \frac{1}{n-1}\sum_{i=1}^n\left(\mathbf{q}_j^\top \mathbf{x}_i\right)^2 \\
        &= \frac{1}{n-1}\Vert\mathbf{X} \mathbf{q}_j\Vert_2^2 \\
        &= \frac{1}{n-1} \mathbf{q}_j^\top \mathbf{X}^\top \mathbf{X} \mathbf{q}_j \\
        &= \frac{1}{n-1} \mathbf{q}_j^\top S \mathbf{q}_j  & (S \text{ is the sample covariance matrix of } \mathbf{X})
      \end{aligned}`$
