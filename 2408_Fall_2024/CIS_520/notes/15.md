...
- Derivation)
  ...
  - Plugging this back into the [General Boosting Algorithm](#concept-general-boosting-algorithm)'s solution, we have   
    $`\begin{aligned}
        ...
        &= \arg\min_{h_{t+1}\in\mathcal{H}} \sum_{i=1}^n r_{i} h_{t+1}(\mathbf{x}_i) & \text{where } r_{i} = H(\mathbf{x}_i) - y_i
    \end{aligned}`$
...
- Prop.)
  - Loss for the boosting $`\ell(H_t + \alpha_{t+1}h_{t+1})`$ is **different** from the loss of getting the next optimal weak learner $`\left( h_{t+1}(\mathbf{x}_i) - (y_i - H_t(\mathbf{x}_i)) \right)^2`$.
    ...
    - We forced $`\ell(H_t + \alpha_{t+1}h_{t+1})`$ to be a squared loss of $`\ell_{\text{sq}}(H) = \frac{1}{2}\sum_{i=1}^n (H(\mathbf{x}_i) - y_i)^2`$ to make the problem solvable.
...
- Def.)
  - $`\displaystyle\ell_{\exp}(H) = \sum_{i=1}^n \exp(-y_i H(\mathbf{x}_i))`$
- Prop.)
...
- Model)
  - $`\displaystyle h_{t+1}^*  = \arg\min_{h_{t+1}\in\mathcal{H}}  \sum_{i : h(\mathbf{x}_i \ne y_i)} w_i`$
    - where
...
- Derivation)
  ...
  - Hence, the optimization can be rewritten as   
    $`\begin{aligned}
      ...
      &= \arg\min_{h_{t+1}\in\mathcal{H}} - \left( -\sum_{i : h(\mathbf{x}_i \ne y_i)} w_i + \sum_{i : h(\mathbf{x}_i = y_i)} w_i \right) \\ 
      &= \arg\min_{h_{t+1}\in\mathcal{H}} \sum_{i : h(\mathbf{x}_i \ne y_i)} w_i - \sum_{i : h(\mathbf{x}_i = y_i)} w_i \\ 
      &= \arg\min_{h_{t+1}\in\mathcal{H}} \sum_{i : h(\mathbf{x}_i \ne y_i)} w_i - \left(1-\sum_{i : h(\mathbf{x}_i \ne y_i)} w_i\right) \quad \left(\because 1 = \sum_i w_i = \sum_{i : h(\mathbf{x}_i \ne y_i)} w_i + \sum_{i : h(\mathbf{x}_i = y_i)} w_i \right) \\
      &= \arg\min_{h_{t+1}\in\mathcal{H}} \sum_{i : h(\mathbf{x}_i \ne y_i)} 2w_i -1 \\
    ...
    \end{aligned}`$ 
  - Here, $`\displaystyle \sum_{i : h(\mathbf{x}_i \ne y_i)} w_i`$ is the weighed classification error.
- Implementation)
  - We should implement $`\displaystyle h_{t+1}^*  = \arg\min_{h_{t+1}\in\mathcal{H}}  \sum_{i : h(\mathbf{x}_i \ne y_i)} w_i`$
  - How?)
    ...
    - $`\displaystyle \frac{\partial }{\partial \alpha} \left(\sum_{i=1}^n \exp(-y_i [H_t(\mathbf{x}_i) + \alpha h_{t+1}(\mathbf{x}_i)])\right) = 0`$.  
      - LHS)   
        $`\begin{aligned}
          ...
          =& \sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i) + \alpha ) - \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i) - \alpha ) & \left(\because y_ih_{t+1}(\mathbf{x}_i)=\begin{cases}1 & \text{if }\mathbf{x}_i=y_i\\-1 & \text{if }\mathbf{x}_i\ne y_i\\\end{cases}\right)
        \end{aligned}`$
      - Thus,   
        $`\begin{aligned}
          &\sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i) + \alpha ) - \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i) - \alpha ) = 0 \\
          \Leftrightarrow& \sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i) ) \cdot\exp(\alpha) - \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i)) \cdot\exp(-\alpha) = 0 \\
          \Leftrightarrow& \exp(\alpha)\left(\sum_{i : h(\mathbf{x}_i) \ne y_i} \exp(-y_iH_t(\mathbf{x}_i) )\right)  = \exp(-\alpha)\left(\sum_{i : h(\mathbf{x}_i) = y_i} \exp(-y_iH_t(\mathbf{x}_i))\right) \\
          \Rightarrow& \exp(2\alpha) = \frac{\displaystyle\sum_{i : h(\mathbf{x}_i) = y_i} \exp(-y_iH_t(\mathbf{x}_i))}{\displaystyle\sum_{i : h(\mathbf{x}_i) \ne y_i} \exp(-y_iH_t(\mathbf{x}_i) )} = \frac{\displaystyle \frac{1}{Z}\sum_{i : h(\mathbf{x}_i) = y_i} \exp(-y_iH_t(\mathbf{x}_i))}{\displaystyle \frac{1}{Z}\sum_{i : h(\mathbf{x}_i) \ne y_i} \exp(-y_iH_t(\mathbf{x}_i) )} = \frac{1-\epsilon_t}{\epsilon_t} \\
