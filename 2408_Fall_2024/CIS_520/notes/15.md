...
- Derivation)
  ...
  - Applying the squared loss, we may get the loss as
    - $`\displaystyle \ell_{\text{sq}}(H) = \frac{1}{2}\sum_{i=1}^n (H_t(\mathbf{x}_i) - y_i)^2 \Rightarrow \frac{\partial \ell_{\text{sq}}}{\partial H_t(\mathbf{x}_i)} = \underbrace{H_t(\mathbf{x}_i) - y_i}_{r_i = \text{residual}}`$.
