    ...
    - $`\displaystyle \frac{\partial }{\partial \alpha} \left(\sum_{i=1}^n \exp(-y_i [H_t(\mathbf{x}_i) + \alpha h_{t+1}(\mathbf{x}_i)])\right) = 0`$.  
      - LHS)   
        $`\begin{aligned}
          ...
          =& \sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i) + \alpha ) - \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i) - \alpha )
        \end{aligned}`$
      - Thus,   
        $`\begin{aligned}
          &\sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i) + \alpha ) - \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i) - \alpha ) = 0 \\
          \Leftrightarrow& \sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i) ) + \sum_{i : h(\mathbf{x}_i \ne y_i)}\alpha - \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i)) + \sum_{i : h(\mathbf{x}_i = y_i)} \alpha = 0 \\
          \Leftrightarrow& \; n\alpha = \sum_{i : h(\mathbf{x}_i = y_i)} \exp(-y_iH_t(\mathbf{x}_i)) - \sum_{i : h(\mathbf{x}_i \ne y_i)} \exp(-y_iH_t(\mathbf{x}_i)) \\
          \Rightarrow& \frac{n\alpha}{Z} = \sum_{i : h(\mathbf{x}_i \ne y_i)} 2w_i -1 \\
          \Rightarrow& \alpha = \frac{Z}{n} \sum_{i : h(\mathbf{x}_i \ne y_i)} 2w_i -1 \\
        \end{aligned}`$ 
