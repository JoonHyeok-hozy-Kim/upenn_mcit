...
#### Prop.) Limits of Kernel Trick
- The options for the kernel function $`k(\mathbf{x}, \mathbf{x}')`$ is limited.
  - e.g.) Linear, Polynomial, RBF, etc
- Making prediction takes sufficient amount of times.
  - e.g.)
    - Let $`\displaystyle \mathbf{w}_* = \sum_{i=1}^n \alpha_i^*\phi(\mathbf{x}_i)`$ be the trained reusult.
    - Then, making prediction with $`\displaystyle h(\mathbf{z}) = \sum_{i=1}^n \alpha_i^* k(\mathbf{x}_i, \mathbf{z})`$ requires $`n`$ kernel computations or $`O(dn)`$ time minimum.
- Most kernels are not good for data modalities.
  - e.g.) 
    - Consider the [RBF Kernel](#eg-the-radial-basis-function-rbf-kernel-gaussian-kernel) that calculates the Euclidean distances.
    - This will make poor predictions for examples like [images with each pixel moved one pixel to right](02.md#2-k-nearest-neighbors).
