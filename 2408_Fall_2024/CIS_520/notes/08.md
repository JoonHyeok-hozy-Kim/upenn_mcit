  ...
  - We will simplify even more by incorporating the two constraints.
    ...
    - pf.)
      ...
      - $`\displaystyle \begin{cases} y_i \mathbf{w}^\top \mathbf{x}_i \gt 0 \\ \min_i \vert \mathbf{w}_*^\top\mathbf{x}_i\vert = 1 \end{cases} \Leftarrow y_i \mathbf{w}^\top \mathbf{x}_i \ge 1`$
        ...
        - Then $`\exists c\in\mathbb{R} \text{ s.t. } \min_i \vert \mathbf{w}_*^\top\mathbf{x}_i\vert \gt c \text{ and } c\ne1`$. 
        - Then, $`c \gt 1.`$
          - Why?)
            - Recall that we assumed that $`y_i\mathbf{w}^\top\mathbf{x}_i \ge 1`$.
            - Also $`y_i\in\{+1,-1\}`$.
            - Thus, $`c \gt 1`$.
        ...
        - If $`\mathbf{w}_{**}`$ is a **feasible solution**, 
          - then   
            ...
            \end{aligned}`$
            - i.e.) $`\mathbf{w}_{**}`$ better optimizes than $`\mathbf{w}_{*}\cdots \otimes`$.
- Analysis)
...
- Application to our SVM)
  - Recall that we are looking for $`\mathbf{w}`$ that satisfies $`y_i \mathbf{w}^\top \mathbf{x}_i \ge 1, \forall i`$.
  - Using the **slack variables** $`\xi_i`$, we will allow some cases that $`y_i \mathbf{w}^\top \mathbf{x}_i \le 1, \exists i`$.
    - How?)
