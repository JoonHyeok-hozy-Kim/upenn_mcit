...
;
;   for (int i=0; i<5; i++){
;     global_array[i] = SUB_FACTORIAL(global_array[i]);
;   }
;
...
FOR_LOOP
  ...
  LDR R0, R2, #0        ; Load the data at global_array[R2] to R0. A = global_array[R2].
  JSR SUB_FACTORIAL     ; Call SUB_FACTORIAL(A). Then B(R1) = SUB_FACTORIAL(A).
  STR R1, R2, #0        ; Overwrite the result. global_array[R2] = B(R1).
