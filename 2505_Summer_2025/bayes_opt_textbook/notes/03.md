...
- e.g.) Scaling Function $`a(x)`$
  ...
  - e.g.) Constant $`a`$ case
    ...
    - Prop.)
      ...
      - Larger $`\lambda \Rightarrow`$ Larger $`K \Rightarrow`$ More variance in output values. (Not [wiggly/smoothness](#concept-design-space-domain-transformation))    
...
- e.g.)
  - Dilation : $`\mathbf{x}\mapsto\mathbf{x}/\ell`$ (i.e. scale by $`\ell^{-1}`$)
    - Props.)
      - Dilated covariance would be $`K'(x,x';\ell) = K(d/\ell) = \displaystyle\exp\left(-\frac{(x-x')^2}{2\ell^2}\right)`$ 
        - where the base covariance $`K(d)`$ on $`\mathcal{X}`$
      - The length scale of unit is $`\ell`$
        - cf.) Recall that the base model's was $`1`$.
      - Longer $`\ell \Rightarrow`$ Larger $`K`$ for the same $`d=\vert x-x'\vert`$   
        $`\quad\quad\quad\Rightarrow`$ More correlation between $`x,x'`$   
        $`\quad\quad\quad\Rightarrow`$ Smoother shape of the sample path.
      - Shorter $`\ell \Rightarrow`$ Smaller $`K`$ for the same $`d=\vert x-x'\vert`$   
        $`\quad\quad\quad\Rightarrow`$ Less correlation between $`x,x'`$   
