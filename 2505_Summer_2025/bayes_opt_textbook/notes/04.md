...
- Settings)
  ...
  - $`\mathcal{M} = \left\{ \left[ p(f\mid\boldsymbol{\theta}), p(y\mid x,\phi,\boldsymbol{\theta}) \right] \mid \boldsymbol{\theta}\in\boldsymbol{\Theta} \right\}`$ : a model space
    - where
...
- Problem)
  ...
  - Committing to a single model in this case may systematically bias our  predictions and underestimate predictive uncertainty
    - Especially when the model posterior is very flat, which is common when observations are scarce.
...
- How?)
  ...
  - Then, we may yield the following simple Monte Carlo estimates   
    ...
    \end{cases}`$
- Limit)
  - Relatively inefficient use of the hyperparameter samples
    - Why?)
      - For each sample of $`\boldsymbol{\theta}`$, we should derive GP posterior.
        - i.e.) $`O(s\cdot n^3)`$ where $`s`$ is the sample size and $`n`$ is the number of data points.
  - Sol.)
    - [Deterministic Approximation Schemes](#concept-deterministic-approximation-schemes)


<br>

### Concept) Deterministic Approximation Schemes
- Approximating $`\boldsymbol{\theta}`$ posterior with a multivariate normal via Laplace approximation
  - Desc.)
    - $`p(\boldsymbol{\theta} \mid \mathcal{D}) \approx\mathcal{N}(\boldsymbol{\theta}; \; \hat{\boldsymbol{\theta}}, \mathbf{C})`$
      - where $`\hat{\boldsymbol{\theta}}`$ is the [MAP](#concept-maximum-a-posteriori-map).
    - Then we may get the averaged model as
      - $`p(f\mid\mathcal{D})\approx \displaystyle\int\mathcal{GP}(f;\mu_\mathcal{D}(\boldsymbol{\theta}), K_\mathcal{D}(\boldsymbol{\theta})) \cdot \mathcal{N}(\boldsymbol{\theta}; \; \hat{\boldsymbol{\theta}}, \mathbf{C}) \;d\boldsymbol{\theta}`$
    - Unfortunately this integral remains intractable due to the nonlinear dependence of the posterior moments on the hyperparameters.
    - But reducing to this common form allows us to derive deterministic approximations against a single assumed posterior.
- Rue et al. (2009), *Approximate Bayesian In ference for Latent Gaussian Models by Using  Integrated Nested Laplace Approximations.*
  - Desc.)
    - Several approximation schemes representing different tradeoffs between efficiency and fidelity
    - Refer to p76-77
- Osborne et al. (2012), *Active Learning of Model Evidence Using Bayesian Quadrature*
  - Desc.)
    - Even more lightweight approximation!
    - Refer to p77-78


<br>

### Concept) Multiple Model Structures
- Goal)
  - Extend the BO framework to handle multiple model structures
- Model Assesment)
  - Discrete union of parametric spaces
    - $`\{\mathcal{M}_i\}`$
      - Each space can be indexed as $`(\boldsymbol{\theta}, \mathcal{M})`$
        - where $`\boldsymbol{\theta}`$ is a vector of hyperparameters associed with the specified model structure 
          - cf.) The size and the interpretation of $`\boldsymbol{\theta}`$ differ across structures.
  - Define a model prior over this compound space as
    - $`p(\boldsymbol{\theta}, \mathcal{M}) = \underbrace{\text{Pr}(\mathcal{M})}_{\text{model structure prior}}\;\cdot\; \underbrace{p(\boldsymbol{\theta} \mid\mathcal{M})}_{\text{hyperparam prior}}`$
  - Then, we may get the model posterior as
    - $`p(\boldsymbol{\theta},\mathcal{M\mid D}) = \text{Pr}(\mathcal{M\mid D}) \;\cdot\; p(\boldsymbol{\theta} \mid\mathcal{D,M})`$
  - Again, we can get the model structure posterior as
    - $`\text{Pr}(\mathcal{M\mid D}) \varpropto \text{Pr}(\mathcal{M}) \cdot p(\mathbf{y\mid x},\mathcal{M})`$
      - where 
        - $`p(\mathbf{y\mid x},\mathcal{M}) = \displaystyle\int p(\mathbf{y\mid x},\boldsymbol{\theta},\mathcal{M}) \; p(\boldsymbol{\theta}\mid\mathcal{M}) \; d\boldsymbol{\theta}`$
          - This integral is in general  intractable, but several approximations are feasible.
          - Laplace approximation or Bayesian Information Criterion(BIC) may be used.
- Model Selection)
  - Since the model space is not continuous with multiple model structures, the gradient methods cannot be used.
  - Instead, we may find the [MAP](#concept-maximum-a-posteriori-map) hyperparameters for each model structures separately, and then use these $`\boldsymbol{\theta}_{\text{MAP}, i}`$ to approximate each structure $`\mathcal{M}_i`$
    - using Laplace approximation or Bayesian Information Criterion(BIC)
- Model Averaging)
  - $`p(f\mid\mathcal{D}) = \displaystyle\sum_i \text{Pr}(\mathcal{M_i\mid D}) \;\cdot\; p(\boldsymbol{\theta} \mid\mathcal{D,M}_i)`$
