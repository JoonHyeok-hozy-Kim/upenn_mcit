        ...
        - Dynamics)
          ...
          - Zero Sum Game
            ...
            - The payoff to $`L`$ goes
              - $`-(\epsilon(h) + \max(0, \vert \epsilon_A(h) - \epsilon_B(h) \vert - \gamma))`$
        - Results)
          - Nash equilibria : Constrained optimization solutions.
      - Classic Theorem (Friend & Schapire)
        ...
        - Analysis)
          - After $`T`$ rounds, the mixture model $`\frac{1}{T}\sum_{t=1}^T h_t`$ is within $`\frac{1}{\sqrt{T}}`$ of the optimal solution to the original constrained optimization problem
            ...
            - Since we introduced the slack variable, the bound became larger.
...
- Problem Applying the **Game Theoretical** Approach on this.
  ...
  - Thus, the regulator $`R`$ has too many $`(i_1, \dots, i_k)`$ to choose : $`2^k`$
...
Bolt-on fairness creates a **straight-line frontier** between the original model and a perfectly fair but high-error model by selectively randomizing subgroup predictions. However, this is often suboptimal: there may exist an **H-Frontier** (also known as a Pareto frontier) that dominates it, offering better fairness-accuracy trade-offs. The optimal region lies **southwest** in fairness-error space.

### 🎲 Output Format:
In the learner-regulator game, the final model is a **uniform mixture** over all models selected by the learner in each round. At prediction time, this means choosing a model at random from this list to make a decision—introducing randomness **globally**, rather than selectively on subgroups as in bolt-on fairness.

### 🧩 Intersectional Fairness Challenge:
Even if fairness constraints are satisfied for individual attributes (e.g., race, gender, age), they may fail **at the intersection** (e.g., disabled Hispanic women over 55). The learner-regulator algorithm can be extended to optimize fairness across **exponentially many intersectional subgroups** efficiently, drawing inspiration from **boosting techniques**.

### ⚖️ Pairwise Fairness:
Average individual fairness can be expressed via **pairwise constraints**—for example, "applicant A and B are similar; if A gets approved, B should not be denied." The algorithm minimizes overall error **subject to these relational constraints**, rather than using group metrics.
