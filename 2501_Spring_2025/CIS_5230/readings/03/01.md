...
### Summary
- Key Fairness Conditions Defined:
  - Calibration within Groups: 
    - For each demographic group, individuals assigned the same risk score should have the same actual probability of a positive outcome. This ensures that the risk scores are accurate and meaningful across all groups.​
  - Balance for the Negative Class: 
    - The average risk score among individuals who do not experience the event (negative class) should be equal across all demographic groups. This prevents any group from being systematically assigned higher risk scores when they do not pose higher risks.​
  - Balance for the Positive Class: 
    - Similarly, the average risk score among individuals who do experience the event (positive class) should be equal across all demographic groups, ensuring no group is unfairly assessed as lower risk when they are not.​

- Main Findings:
  - The authors demonstrate that it is generally **impossible to satisfy all three fairness conditions simultaneously**, except in specific cases:​
    - **Perfect Prediction**: 
      - Where the model predicts outcomes with complete accuracy (i.e., no uncertainty).​
    - **Equal Base Rates**: 
      - Where the proportion of individuals experiencing the event is identical across all demographic groups.​
  - Outside these scenarios, enforcing one fairness condition may lead to the violation of others, highlighting inherent trade-offs in designing fair risk assessment tools.​

- Implications:
  - This work provides a framework for understanding the limitations and necessary compromises when developing fair algorithmic risk scores. It emphasizes that stakeholders must carefully consider which fairness criteria to prioritize, as achieving all simultaneously may not be feasible.​
  - In summary, the paper sheds light on the complex interplay between different notions of fairness in risk assessments and underscores the importance of acknowledging and navigating these trade-offs in practical applications.
