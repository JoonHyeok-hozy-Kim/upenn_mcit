...
#### e.g.) Merge Sort
- Suppose we use thread to speed up the merge sort as below...
  ```cpp
  void merge_sort(int[] arr, int lo, int hi) {
    int mid = (lo + hi) / 2;

    pthread_create(merge_sort(arr, lo, mid)); // Thread!
  ...
  ```
- Let
  - $`T(n)`$ : the running time of our algorithm
  - `merge` operation takes $`O(n)`$ running time.
- Then, the running time can be rewritten as...   
  $`\begin{aligned}
    T(n) &= \underbrace{O(n)}_{\text{merge}} + T(n/2) & (\because \text{the other } n/2 \text{ runs in parallel}) \\
    &= O(n) + \underbrace{O(n/2)}_{\text{merge}} + T(n/4) & (\because \text{the other } n/4 \text{ runs in parallel})  \\
    &= O(n) + O(n/2) + \underbrace{O(n/4)}_{\text{merge}} + T(n/8) & (\because \text{the other } n/8 \text{ runs in parallel})  \\
    &\vdots \\
    &= O(n) + O(n/2) + \cdots + O(1) \\
    &= O\left( n \left( 1+\frac{1}{2}+\frac{1}{4}+\cdots\right)\right) \\
    &= O(2n) = O(n)
  \end{aligned}`$
...
- Let $`T_1`$ be a parallel algorithm s.t. $`T_1 = (1-S) + S`$ where $`S`$ is the part that can be parallelized.
- For $`P`$ the number of threads that are perfectly linear speeding up on the parallelizable part, we may get
  - $`\displaystyle T_p = (1-S) + \frac{S}{P}`$
- Thus, $`\displaystyle \frac{T_1}{T_p} = \frac{1}{1-S + \frac{S}{P}}`$
- Suppose we have $`100,000`$ threads. (HUGE!)
  - Then $`\displaystyle \frac{T_1}{T_p} = \frac{1}{1-S + \frac{S}{100,000}}`$
- Depending on the portion that can be parallelized,   
  |$`S`$|$`\displaystyle\frac{T_1}{T_p}`$|
  |:-:|:-:|
  |$`\frac{2}{3}`$|$`2.99`$|
  |$`0.9`$|$`9.99`$|
  |$`0.99`$|$`99.99`$|
