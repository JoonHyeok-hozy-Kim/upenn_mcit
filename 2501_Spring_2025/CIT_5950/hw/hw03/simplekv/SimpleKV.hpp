...
class SimpleKV {
 public:
  ...
  bool validate_idx(const std::string& nspace, const std::string& key, size_t idx);
 ...
 private:
  ...
  std::map<std::string, std::unordered_map<std::string, std::variant<std::string, list>>> simple_namespace {};
