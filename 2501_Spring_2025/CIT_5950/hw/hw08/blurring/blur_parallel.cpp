...
void* get_batch_avg_RGB(void* arg) {
	...
	UINT block_size = args->shared_args->block_size;
	UINT height = args->shared_args->height;
	UINT width = args->shared_args->width;

	UINT thrd_num = args->shared_args->thrd_num;
	...
	while (y < height) {
		...
		for (int idx = 0; idx < num_iter; idx++) {
			...
			for (int jdx = 0; jdx < num_iter; jdx++) {
				...
				RGB from_pixel = args->shared_args->from_image->get_pixel(xx, yy);
		...
		args->shared_args->to_image->set_pixel(x, y, avg_rgb);
		++(*pxl_cnt);
...
int main(int argc, char* argv[]) {
  ...
  SharedArgs* shared_args = new SharedArgs();
  shared_args->from_image = &image;
  shared_args->to_image = &blurry;
  shared_args->height = height;
  shared_args->width = width;
  shared_args->block_size = block_size;
  shared_args->thrd_num = static_cast<UINT>(thrd_num);
  ...
  for (int idx = 0; idx < thrd_num; ++idx) {
	 ...
	 thread_args->shared_args = shared_args;
	 thread_args->mod = idx;

	 if (pthread_create(&thrd, nullptr, get_batch_avg_RGB, static_cast<void*>(thread_args)) != 0) { 
